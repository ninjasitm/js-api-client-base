  ...
  addToStateData(state, originalItem, stateIsTarget, push) {
    const addData = function (_state, _item) {
      if (_state instanceof Array) {
        ...
        const index = filteredState.findIndex(current => current instanceof Object && current.id === _item.id);
        if (index > -1) {
          const existing = filteredState.find(current => current instanceof Object && current.id === _item.id);
    ...
    const items = originalItem instanceof Array ? originalItem : [originalItem];
    items.forEach(item => {
